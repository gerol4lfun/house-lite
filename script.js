// ===== 1. Константы =====
ymaps.ready(init);          // ждём, пока Я-карты полностью загрузятся
function init() {
...
const INREP   = {
  osb_vag:120, osb_imit:350, vag_imitBC:250,
...
// ===== 4. Сваи + добавление строк окон/дверей =====
...
btnCalc.onclick = async ()=>{
  ...
  let km = await getKm(addr); if(km===null)return;
  ...
  Array.from(selDoors.selectedOptions).forEach(opt=>{
    add(DOORS[opt.value], opt.text.split('(')[0]);
  });
  ...
  document.querySelectorAll('.window-row').forEach(row=>{
    ...
    if(price) add(price*qty, `${qty}× ${type==='pvhdoor'?'Дверь ПВХ':'Окно'} ${sz}` + (type==='window'?` ${cam}-камерное`:``));
  });
  ...
  if(extras>0){
    outLines.push(`– Доп. услуги: ${extras.toLocaleString()} ₽`, ...linesExtra);
  }
  outLines.push(
...
async function getKm(address){
  try{
    ...
    const route = await ymaps.route([[55.751244,37.618423], obj.geometry.getCoordinates()]);
    return route.getLength()/1000;
  }catch(e){
    alert('Ошибка Яндекс.Карт'); console.error(e);
    return null;
  }
}
}