...
const WINDOWS = {
  '50×50':{1:5500,2:7000},   '60×90':{1:7500},
  '60×120':{1:10000},        '60×180':{1:12000,2:17000},
  '100×100':{1:9000},        '100×120':{1:11000,2:14000},
  '120×120':{1:13000},       '100×140':{1:14000},
  '100×150':{1:15000},       '120×150':{1:16500},
  '140×150':{1:17000},       '150×150':{1:17500},
  '150×100':{1:15000},       '150×180':{2:25000},
  '150×190':{1:25000},       '180×190':{1:26000},
...
function populatePileOptions(){
  const w   = +inpWidth.value,
        l   = +inpLength.value,
        key = `${w}x${l}`,
        cnt = PILE_COUNT[key] || 12;

  selPile.innerHTML = '<option value="">— без свай —</option>';
  for (const [dim, price] of Object.entries(PILES)){
    selPile.innerHTML +=
...
function addWindowRow(){
  const row = tmplWindowRow.content.cloneNode(true).firstElementChild,
        selType = row.querySelector('.win-type'),
        selCam  = row.querySelector('.win-cam'),
        selSz   = row.querySelector('.win-size');

  const rebuild = () => {
    const cam    = selCam.value,
          isDoor = selType.value === 'pvhdoor';

    selSz.innerHTML = '<option value="">— размер —</option>';
    for (const [size, cams] of Object.entries(WINDOWS)){
      if (isDoor){
        if (size.includes('дверь ПВХ')){
          selSz.innerHTML += `<option value="${size}">${size} (${(cams[2]||cams[1]).toLocaleString()} ₽)</option>`;
        }
      }else if (cams[cam]){
  ...
  row.querySelector('.btnRemoveWindow').onclick = () => row.remove();
  rebuild();
...
async function calculate(){
  ...
  if (selInsul.value !== 'none')   add(INSUL  [selInsul.value]   * area, selInsul.selectedOptions[0].text.split('(')[0]);
  if (selRoofMat.value !== 'none') add(ROOFMAT[selRoofMat.value] * area, selRoofMat.selectedOptions[0].text.split('(')[0]);
  if (selVeranda.value !== 'none') add(VERANDA[selVeranda.value] * area, selVeranda.selectedOptions[0].text.split('(')[0]);
  if (selInRep.value !== 'none')   add(INREP  [selInRep.value]   * area, selInRep.selectedOptions[0].text.split('(')[0]);
  if (selOutRep.value !== 'none')  add(OUTREP [selOutRep.value]  * area, selOutRep.selectedOptions[0].text.split('(')[0]);
  ...
  document.querySelectorAll('.window-row').forEach(row=>{
    const type = row.querySelector('.win-type').value,
          cam  = row.querySelector('.win-cam').value,
          sz   = row.querySelector('.win-size').value,
          qty  = +(row.querySelector('.win-qty').value || 0);

    if (!sz || qty < 1) return;

    const price = type === 'pvhdoor'
      ? (WINDOWS[sz][2] || WINDOWS[sz][1])
      : WINDOWS[sz][cam];
  ...
  const lines = [
    `🏠 Дом ${w}×${l} м (${roof==='lom' ? 'ломаная' : 'двускатная'} крыша)`,
    ``,
    `💰 Стоимость:`,
    `– Базовая комплектация: ${basePrice.toLocaleString()} ₽`,
  ...
  lines.push(
    ...
    ``,
    `🎁 Подарки: фундамент из блоков, сборка за 1 день, обработка антисептиком`,
    ``,
    `🕒 Срок: 3–7 дней    |    💳 Без предоплаты — расчёт по факту`
  );
